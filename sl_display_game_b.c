...
/*   Updated: 2022/01/19 18:45:02 by marugula         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	display_img2(t_mlx *mlx, char ch, int x, int y)
{
	...
	{
		if (n % 4 == 0)
			put_img(*mlx, mlx->imgs.coin1.img, x, y);
		if (n % 4 == 1)
			put_img(*mlx, mlx->imgs.coin2.img, x, y);
		if (n % 4 == 2)
			put_img(*mlx, mlx->imgs.coin3.img, x, y);
		if (n % 4 == 3)
			put_img(*mlx, mlx->imgs.coin4.img, x, y);
		n++;
	}
}
...
void	display_img(t_mlx *mlx, char ch, int x, int y)
{
	if (ch == '1')
	{
		if (ft_rand() % 3 == 0)
			...
			put_img(*mlx, mlx->imgs.wall2.img, x, y);
		else
			put_img(*mlx, mlx->imgs.wall3.img, x, y);
	}
	else if (ch == 'E')
...
t_img	fill_imgs(void *mlx)
{
	...
	imgs.ant.img = mlx_xpm_file_to_image(mlx, \
		"img/ant.xpm", &imgs.ant.width, &imgs.ant.heigth);
	...
	imgs.coin2.img = mlx_xpm_file_to_image(mlx, \
		"img/coin2.xpm", &imgs.coin2.width, &imgs.coin2.heigth);
